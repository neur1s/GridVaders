...
class NBackDataset(ItaloNBackDataset):  # Inherit from Italo's class
    def __init__(self, num_samples, n_back):
        ...
        dataset = create_n_back_dataset(
            ...
            n=n_back,
            max_length=seq_length + n_back,  # Ensure enough targets
            grid_size=grid_size,
            boundary=boundary
        )
...
class NBackLSTM(nn.Module):
    def __init__(self, hidden_state_size, num_layers=1, grid_size=np.array([5, 5], dtype=int), dropout=0.2):
        ...
        # Initialize hidden and cell states as parameters
        self.h0 = nn.Parameter(torch.randn(num_layers, 1, hidden_state_size) * 0.05)
        self.c0 = nn.Parameter(torch.randn(num_layers, 1, hidden_state_size) * 0.05)
    ...
    def forward(self, X, return_hidden=False):
        ...
        c0 = self.c0.expand(self.num_layers, batch_size, -1).contiguous()
        
        states, _ = self.core(X, (h0, c0))
...
def sample_and_train(n_back, hidden_units, num_layers=1, 
    ...
    model = NBackLSTM(hidden_units, num_layers=num_layers).to(device)
